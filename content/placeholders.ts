...
export const placeholderRegistry: PlaceholderEntry[] = [
  ...
  },
  {
    ...
    status: "pending-approval",
    description: "Production Recovery: the contact backend was replaced with a minimal, direct implementation — Resend only, called directly, no provider-abstraction layer, no rate limiting (Upstash Redis and lib/rate-limit.ts removed entirely; this was a deliberate simplification to restore a working production contact form, not an oversight — reintroducing rate limiting later would need a fresh design, not a revival of the removed code). The two-email workflow is unchanged: an internal notification to CONTACT_INTERNAL_NOTIFICATION_EMAILS (defaults to haluk@mbkholding.qa, tariq@mbkholding.qa), then an automatic acknowledgement to the visitor. Remaining work: (1) DEFERRED, not currently blocking — Resend sender-domain verification and the final RESEND_FROM_EMAIL value depend on a production domain that hasn't been fully verified with Resend yet (see production-domain). (2) STILL AN ACTUAL BLOCKER — the Privacy Policy must receive legal approval, since the form's consent checkbox depends on it regardless of domain/email status.",
    location: "lib/actions.ts, lib/email-provider.ts, .env.example",
  },
  {
